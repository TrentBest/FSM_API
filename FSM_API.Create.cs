...
{
	/// <summary>
	/// Provides methods for defining new Finite State Machine structures and creating
	/// live instances of those definitions. This section is dedicated to the setup
	/// and instantiation of FSMs within the application.
	/// </summary>
	/// <remarks>
	/// This partial class, alongside others, forms the comprehensive <see cref="FSM_API"/>.
	/// Methods within this section are typically the starting point for integrating FSM
	/// logic, allowing you to establish FSM blueprints and then bring them to life
	/// for specific contextual objects.
	/// </remarks>
	public static partial class FSM_API
	{
			...
			int processRate = 0, string processingGroup = "Update")
		{
			...
			if (buckets.TryGetValue(fsmName, out var existingBucket))
			{
				...
				return new FSMBuilder(existingBucket.Definition != null ? existingBucket.Definition : _defaultFSM);
			}
