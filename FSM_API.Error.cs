...
{
	/// <summary>
	/// This partial class defines the public configuration properties and error reporting mechanisms
	/// for the <see cref="FSM_API"/>.
	/// </summary>
	/// <remarks>
	/// This part of the API focuses on allowing consumers to configure how errors are handled
	/// and to subscribe to internal diagnostic events.
	/// <para>
	/// **Important:** This API is designed for **single-threaded use**. All calls to public
	/// and internal methods, especially those that modify FSM definitions or instances,
	/// are expected to originate from a single thread (e.g., Unity's main thread, a game loop thread).
	/// Attempting concurrent modifications from multiple threads will lead to undefined behavior
	/// and potential data corruption, as no internal locking mechanisms are employed to prioritize performance.
	/// </para>
	/// </remarks>
	public static partial class FSM_API
	{
        /// <summary>
        /// Tracks the number of errors reported for each active FSM instance.
        /// Key: <see cref="FSMHandle"/>, Value: Error Count.
        /// </summary>
        /// <remarks>
        /// Used in conjunction with <see cref="ErrorCountThreshold"/> to automatically
        /// unregister problematic FSM instances.
        /// Consider using <see cref="System.Collections.Concurrent.ConcurrentDictionary{TKey, TValue}"/>
        /// for thread safety if errors can be reported from different threads.
        /// </remarks>
